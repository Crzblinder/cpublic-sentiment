...
| 舆情采集 | 21 个公开数据源（7 个稳定 RSS + 2 个 API + 12 个备选），异步并发采集，支持增量去重 |
...
│   │   ├── api/             # REST API（含 SSE 流式端点 + 爬虫触发接口）
│   │   ├── crawler/         # 舆情采集（21 数据源，RSS/API 双模式，增量去重）
...
系统内置 21 个公开数据源，其中 **7 个 RSS 源稳定可用**：

| 源名称 | 类型 | 说明 |
|--------|------|------|
| chinanews_rss | RSS | 中国新闻网滚动新闻 |
| people_rss | RSS | 人民网时政新闻 |
| xinhua_rss | RSS | 新华网时政新闻 |
| sspai_rss | RSS | 少数派（科技资讯） |
| ithome_rss | RSS | IT之家（科技新闻） |
| jiemian_rss | RSS | 界面新闻（财经/商业） |
| 36kr_rss | RSS | 36氪（创投/科技） |

每次采集约 190 条新闻，经 7 步清洗管线（HTML 去标签 → 文本规范化 → SimHash 去重 → 实体提取 → 风险分类 → 等级评估 → 行业归类）后持久化入库。
...
> 如需扩展数据源，编辑 `backend/app/crawler/sources.py`，按模板添加 RSS/API 源配置即可。
...
| POST | `/api/v1/crawler/run` | 触发舆情采集（并发抓取所有数据源） |
| GET  | `/api/v1/crawler/status` | 爬虫状态（采集量、成功/失败源） |
